...
|---|---|---|
| Verdict engine + E2E | `node site/audit-page/test.mjs` | 207 |
| Adversarial extractors (audit + built checker) | `node site/audit-page/test-adversarial.mjs` | 170 |
| Checker migration reports (built page) | `node site/test-checker.mjs` | 57 |
...
| `../test-checker.mjs` | 57 assertions for the **checker's** `migrate()` and its Excel-for-the-web helpers (`xwWhy`, `xwDate`, `xwDateLabel`, the unmeasured block), loaded out of the built `docs/checker.html` the same way `test-adversarial.mjs` loads `funcs()`. Rebuild first. `node site/test-checker.mjs`. |
...
- **Checker migration targets** (`docs/checker.html`, generated by `CHECKER_TMPL` in
  `build_site.py`; permalink param `&t=`): `x` = Excel (desktop), `xw` = Excel for the
  web, `g` = Google Sheets, `l` = LibreOffice (+ `&v=` for the release). Three of the
  four are execution targets — `l`/`g`/`xw` decide from `lv`/`gv`/`xwv` — and only `x`
  is documentation-based, which its own "Target:" line says. The checker's report now
  opens with a `Target:` line naming the basis, matching the audit page's report header;
  for `xw` that line and the `Migration to …` heading carry the **executed date label**
  (each function's own `xwver` date, or the span when a report covers functions from
  more than one run; the file-level run date is used only to say when the most recent
  run was, and is labelled as such).
- **Checker vs audit on a null `xwv` — a deliberate difference.** The audit page returns
  UNKNOWN and stops (see the honesty note below). The checker instead falls back to the
  documented **desktop** flag `d.x` so the migration report can still be useful, and
  then lists every function decided that way in its own "could not be checked against a
  run of Excel for the web" block, saying in as many words that the basis was
  documentation for desktop Excel and not a measurement of the web app. The fallback is
  therefore never silent, and the top-of-page summary line ("Excel for the web
  (executed): works where we measured it") does not use it at all. The seven
  `EXCEL_WEB_TRANSPORT_SKIPS` functions get **no** fallback in either tool: the checker
  blocks them and says the workbook was refused, rather than inferring anything.
