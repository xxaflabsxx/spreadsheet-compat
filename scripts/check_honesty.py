...
So this script fails (exit 1) on two kinds of dishonesty:
  ...
  3. SELF-CONTRADICTORY RECIPE COPY -- a how-to page that shows an executed
...
# (3) A recipe page cannot both show an executed Google Sheets result and
...
for f, ctx in contradictory[:40]:
    print(f"    {f}: …{ctx}…")
sys.exit(1 if (bad or stale or contradictory) else 0)
